...
public class ButtonChooseLocation extends Button{
	...
	public ButtonChooseLocation(float x, float y, float width, float height,
...
//		Helper.println("Checking hitpoint...");
		if(this.checkHit(p)){
			isTouched = true;
			Helper.println("go to chooseplayerandball...");
//			this.setImage(Helper.getImageFromAssets(AssetConstants.IMG_OPTIONS_PRESSED));
			GlobalVars.ge.loadStage(new ChooseLocation());			
		}
	}

	public ButtonChooseLocation(float x, float y, float width, float height, int zIndex) {
