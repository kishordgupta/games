...
//			Helper.println("Image already loaded, loading from file: " + imageConstant);
	...
	public static OrthographicCamera getCamera() {
		return GlobalVars.ge.getScreen().cam;
	}
	
	public static float getCameraX() {
		return GlobalVars.ge.getScreen().cam.position.x-Helper.getScreenWidth()/2f;
	}
	
	public static float getCameraY() {
		return GlobalVars.ge.getScreen().cam.position.y-Helper.getScreenHeight()/2f;
	}
	
	public static void setCameraX(float x) {
		GlobalVars.ge.getScreen().cam.position.x = x + Helper.getScreenWidth()/2f;
	}
	
	public static void setCameraY(float y) {
		GlobalVars.ge.getScreen().cam.position.y = y + Helper.getScreenHeight()/2f;
	}

	public static int getScreenScale() {		
		if(Helper.getScreenHeight() > Helper.getScreenWidth()){
			...
			else if(Gdx.graphics.getHeight() > 350)
				return 2;
			else
				return 1;
		}
