...
public class AniPinBall extends AnimationBase {
	...
	public void step(long stepTime) {
		...
		if(this.element.getX() >= Gdx.graphics.getWidth()-this.element.getWidth()/2f || this.element.getX() <= 0)
		{
			this.dx *=-1;
		}
		if(this.element.getY() >= Gdx.graphics.getHeight()- this.element.getHeight() || this.element.getY() <= 0)
		{
			this.dy *=-1;
		}
	}
