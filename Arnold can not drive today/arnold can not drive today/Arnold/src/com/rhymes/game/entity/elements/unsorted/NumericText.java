...
public class NumericText extends ElementBase{
	...
	public void renderScore() {
		...
		if(number < 0){
			...
			GlobalVars.ge.getRenderer().render(selectFont('m'), x, y - height, width, height);
			digitWidth = width + charsPadding['0' - '0'];
//			x = x + digitWidth;
...
//			Helper.println("d[i]: " + d[i]);
	...
	public TextureRegion selectFont(char a) {
		...
		if (a == '0')
			...
			texture = Helper.getImageFromAssets(AssetConstants.FONT_MINUS);
	...
	public void getAssets(AssetPack assetPack) {
		...
		assetPack.addTexture(AssetConstants.FONT_MINUS);
	}
