...
public class GameOverScreen extends StageBase {
	...
	public void loadScore(Result res){
		...
		if(res instanceof ResultBTRClassic){
			...
			for(i = 0; i < levelStarNumber; i++){
				
				
				if(starCollected > 0){
					for(j = 0; j < starCollected; j++){
						star[i] = new ButtonStar(x*GameMenuInfo.ratio_wl, y*GameMenuInfo.ratio_hl, Helper.getImageFromAssets(AssetConstants.IMG_STARS).getRegionWidth()/10*GameMenuInfo.ratio_w_img, Helper.getImageFromAssets(AssetConstants.IMG_STARS).getRegionHeight()/10*GameMenuInfo.ratio_h_img, 1, AssetConstants.IMG_STARS);
						this.elements.add(star[i]);
						
						i++;
//						starCollected--;
						y -= (Helper.getImageFromAssets(AssetConstants.IMG_STARS).getRegionWidth()/10-5);
					}
				}
				
				star[i] = new ButtonStar(x*GameMenuInfo.ratio_wl, y*GameMenuInfo.ratio_hl, Helper.getImageFromAssets(AssetConstants.IMG_EMPTY_STAR).getRegionWidth()/10*GameMenuInfo.ratio_w_img, Helper.getImageFromAssets(AssetConstants.IMG_EMPTY_STAR).getRegionHeight()/10*GameMenuInfo.ratio_h_img, 1, AssetConstants.IMG_EMPTY_STAR);
				this.elements.add(star[i]);
...
//						starCollected--;
				...
				star[i] = new ButtonStar(x, y, Helper.getImageFromAssets(AssetConstants.IMG_EMPTY_STAR).getRegionWidth()/10*GameMenuInfo.ratio_w_img, Helper.getImageFromAssets(AssetConstants.IMG_EMPTY_STAR).getRegionHeight()/10*GameMenuInfo.ratio_h_img, 1, AssetConstants.IMG_EMPTY_STAR);
				this.elements.add(star[i]);
...
//			y = 290;
			...
			score_result.setNumber(pearlCollected/levelPearlNumber * 100);
