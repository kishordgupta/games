...
//	protected LinkedList<Point> segmentPoints;
	...
	protected int renderingSegmentSize = 14;
	...
	public void renderPoints(TextureRegion ropeTexture) {
		...
		for (int i = 0; i < nodes.size()-1; i++) {
			if(nodes.get(i).isActive()){
				hp.setLocation(nodes.get(i).getX() , 
						nodes.get(i).getY());
				
				jp.setLocation(nodes.get(i+1).getX() , 
						nodes.get(i+1).getY());
				
//				hp.setLocation(nodes.get(i).getX(), nodes.get(i).getY());
//				jp.setLocation(nodes.get(i+1).getX(), nodes.get(i+1).getY());
				
				pathRenderer.renderPathSegment(hp, jp,
					 ropeTexture,
					 renderWidth );
			}
			else{
...
//		System.out.println("" + (System.currentTimeMillis() - time));
	...
	public void renderSimulation(TextureRegion ropeNormalImage,
		...
		for (int i = 0; i < nodes.size() - 1; i++) {
			if (nodes.get(i).isActive())
				pathRenderer.renderPathSegmentSimulation(nodes.get(i), nodes.get(i + 1),
						ropeBurnedImage);
			else
				pathRenderer.renderPathSegmentSimulation(nodes.get(i), nodes.get(i + 1),
						ropeNormalImage);
...
//					+ r.getY() + " )");
	...
	public TraversePointInfo startTraverse(Point hitPoint, float maxDistance) {
		...
		nodes = renderNodes;
		PathNode t = null;
//		hitPoint.x  *= Helper.ratioX;
//		hitPoint.y  = Helper.getAdjustedY(hitPoint.y);
		for (int i = 0; i < nodes.size(); i++) {
			p.setLocation(nodes.get(i).getLocation().getX(),nodes.get(i).getLocation().getY());
			distance = p.distancePoint2Point(hitPoint);
...
//		Helper.printKeyVal("Total Distance Set: ", info.getTotalDistanceInPath());
	...
	public boolean checkRange(Point hitPoint, float maxDistance) {
		Helper.println("\n\n\nChecking Range: Nodes size: " + nodes.size());
		float distance, d;
		for (int i = 0; i < nodes.size() - 1; i++) {
//			distance =  (float) Helper.pointToLineDistance(nodes.get(i).getLocation(), nodes
//					.get(i + 1).getLocation(), hitPoint);
			distance = hitPoint.distancePoint2Point(nodes.get(i).getLocation());
			
			if (distance <= maxDistance)
			{
				Helper.println("node in range: " + nodes.get(i)+ "  to  " + nodes.get(i+1) + " distance: " + distance);
				return true;
			}
		}
		return false;
...
//				Helper.printKeyVal("Total Distance Set: ", info.getTotalDistanceInPath());
	...
	public TraversePointInfo startTraverse()
	{
		...
		info.setDistance(0);
		info.setTotalDistanceInPath(calculateTotalDistance());
		info.setPath(this);
		return info;
	}
