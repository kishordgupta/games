...
public class ProgressBar extends ElementBase{
	...
	public void render() {
		...
		if(!fadeProgressBar){
			...
			GlobalVars.ge.getRenderer().render(barImage, x+0 * LevelInfo.ratioX, y, w < 0 ? 0:width * progress - 40 * LevelInfo.ratioX, height);
			GlobalVars.ge.getScreen().getBatch().setColor(Color.WHITE);
		}
		else{
//			Helper.printTest("progress: " + progress);
			if(progress <= 0.5){
				GlobalVars.ge.getScreen().getBatch().setColor(r, g, b,1-progress);
				GlobalVars.ge.getRenderer().render(backImage, x, y, width, height);
			}
			GlobalVars.ge.getScreen().getBatch().setColor(0.2f, 0.2f, 0.2f,progress);
			GlobalVars.ge.getRenderer().render(barImage, x, y, width, height);
			GlobalVars.ge.getScreen().getBatch().setColor(Color.WHITE);
		}
	...
	private boolean fadeProgressBar = false;
			 ...
			 boolean fadeProgressBar)
	{
		...
		this.fontName = fontName;
		this.prefix = prefix;
		this.fadeProgressBar = fadeProgressBar;
	}
