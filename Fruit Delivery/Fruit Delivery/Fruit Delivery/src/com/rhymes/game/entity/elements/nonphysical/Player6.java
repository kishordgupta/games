...
public class Player6 extends Player {
	...
	public Player6(float x, float y, float width, float height,
			float rotateAngle, int zIndex) {
		super(x, y, width, height, rotateAngle, zIndex);
	}

	public Player6(float x, float y, float width, float height, int zIndex) {
	...
	public void init() {
		...
		imgNormal[0] = Helper.getImageFromAssets(AssetConstants.IMG_PLAYER6_NORMAL1);
		imgNormal[1] = Helper.getImageFromAssets(AssetConstants.IMG_PLAYER6_NORMAL2);
		imgThrow[0] = Helper.getImageFromAssets(AssetConstants.IMG_PLAYER6_THROW1);
		imgThrow[1] = Helper.getImageFromAssets(AssetConstants.IMG_PLAYER6_THROW2);
		imgEnd[0] = Helper.getImageFromAssets(AssetConstants.IMG_PLAYER6_END1);
		imgEnd[1] = Helper.getImageFromAssets(AssetConstants.IMG_PLAYER6_END2);
