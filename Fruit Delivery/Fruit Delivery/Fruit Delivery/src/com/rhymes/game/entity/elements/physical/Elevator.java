...
public class Elevator extends ElementBase {
	...
	TextureRegion texturePathCarpet;
	TextureRegion texturePathTexture;
	...
	public Elevator(World world, int COUNT, Vector2 startPoint,
			Vector2 endPoint, float width, float height, int zIndex,
			String imagePathCarpet, String imagePathTexture) {
		this.width = width;
		this.height = height;
		this.elevetorWidth = PhysicsHelper.ConvertToBox(width);
		this.elevetorHeight = PhysicsHelper.ConvertToBox(height);
...
//	if(Math.abs(p.getX() - GlobalVars.ge.getScreen().cam.position.x) > Gdx.graphics.getWidth())
//		continue;
//	if(Math.abs(p.getY() - GlobalVars.ge.getScreen().cam.position.y) > Gdx.graphics.getHeight())
//		continue;

				if(q.x < Helper.getCameraLeft() && p.x < Helper.getCameraLeft())
					continue;
				else if(p.x > Helper.getCameraRight() && q.x > Helper.getCameraRight())
					continue;
...
//							15* LevelInfo.ratioX, f);	
					GlobalVars.ge.getRenderer().render(texturePathTexture,
							r.x, r.y - f,
							15* LevelInfo.ratioX, f, 
							0,texturePathTexture.getRegionHeight() - f,
							texturePathTexture.getRegionWidth(), f);
				}
				
				
				GlobalVars.ge.getRenderer().render(texturePathCarpet, p.x,
						p.y - this.height/2f, distance,
						/* GameMenuInfo.ratio_h* */this.height * 2f, 0,
						this.height, Helper.getAngle(p, q) + 90, 1, 1);
...
//				GlobalVars.ge.getScreen().getBatch().setColor(1f, 1f, 1f, 1f);
	...
	@Override
	public void init() {
//		this.imageelevator = Helper.getImageFromAssets(CSPConstants.TROPIC_CARPET);
		this.texturePathCarpet= Helper.getImageFromAssets(imagePathCarpet);
		this.texturePathTexture = Helper.getImageFromAssets(imagePathTexture);
	}

	@Override
	public void getAssets(AssetPack assetPack) {
		assetPack.addTexture(AssetConstants.IMG_AXLE);
		assetPack.addTexture(imagePathCarpet);
		assetPack.addTexture(imagePathTexture);
	}
