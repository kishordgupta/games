...
//	private static final XLevel Xlevel = null;
	...
	public static void main(String filePath, float af, float bf) {
		ropes.clear();
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				for (int i = 0; i < RopeNode.getLength(); i++) {
					...
					if (attr.compareTo(Num) == 0) {
						...
						float a = (Float.valueOf(attribX)).floatValue() *2f;
						float b = Gdx.graphics.getHeight()
								- (Float.valueOf(attribY)).floatValue()-250f ;
			...
			//	String Ropecolor = eElement.getAttribute("Color");
				...
				for (int i = 0; i < RopeNode.getLength(); i++) {
					...
					if (attr.compareTo(Num) == 0) {
						...
						RopeNodeS.setX(a * 1.6f );
						RopeNodeS.setY(b * 1.5f );
						
//						RopeNodeS.setX(a * 2f);
//						RopeNodeS.setY(b * 1.0f);
